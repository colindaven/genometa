...
\begin{tabular}{|p{7cm}|p{7cm}|}
\hline
\multicolumn{2}{|c|}{\textbf{Feature annotation track}}\\
\hline
\textbf{Features read in from a GFF file should be displayed as horizontal bars together with their orientation,
positive or negative.}
&
\begin{itemize}
	\item the software should display genes at their corresponding start and end positions
	\item the software should stack overlapping genes to allow distinction
\end{itemize}
\\
\hline
\end{tabular}
...
&
\begin{itemize}
	\item show numbers of reads attributed to each species/gene in text widget
	\begin{itemize}
	\item Metagenomics: Normalise number of reads which hit each genome by genome length
  \item Transcriptomics: Normalise number of reads which hit each gene by gene length to 1000bp
and normalise total number of reads to one million (Reads per Kilobase per Million RPKM).
\end{itemize}
\end{itemize}
\\
\hline
...
\begin{tabular}{|p{7cm}|p{7cm}|}
\hline
\multicolumn{2}{|c|}{\textbf{Run external alignment tools and reimport results}}\\
\hline
\textbf{Run alignment of specified reads file (in fastA, fastQ format) against a built reference database of
bacterial species.}
&
\begin{itemize}
	\item the software should include funtionality to run an external aligment tool
	\begin{itemize}
	\item the user should be able to choose program used
	\item options for the external aligner should be settable via ui interaction
	\item progress of external alignment should be visible to the user
	\item as most external aligners are linux based this functionality should be linux only
\end{itemize}
\end{itemize}
\\
\hline
...
&
\begin{itemize}
	\item export sequence of DNA ACTG to a text widget
\end{itemize}
\\
\hline
\end{tabular}
...
\begin{tabular}{|p{7cm}|p{7cm}|}
\hline
\multicolumn{2}{|c|}{\textbf{Analysis for transcriptomes}}\\
\hline
\textbf{Find reads that group together into continuous segments and list them as
new potential sRNAs or ORFs.}
&
\begin{itemize}
